...
def get_redis_url(db, redis=None):
    ...
    >>> get_redis_url(1, {'host':'host1:26379,host2:26379', 'sentinel_master_set':'mymaster', 'password':'password1','idle_timeout_seconds':30,'sentinel_password':'password2'})
    'redis+sentinel://anonymous:password@host1:26379,host2:26379/mymaster/1?idle_timeout_seconds=30&sentinel_password=password2'
...
def get_redis_url_param(redis=None):
    ...
    if 'idle_timeout_seconds' in redis:
        s['idle_timeout_seconds'] = redis['idle_timeout_seconds']
    if 'sentinel_password' in redis:
        s['sentinel_password'] = redis['sentinel_password']
    if s:
        return "?" + urlencode(s)
    return ""
